...
export const axiosPost = (url, params, fn) => {
    ...
    axios.post(_url, params, {
        ...
        if (error.response) {
            if (/^(5|4)\d{2}/.test(error.response.status)) {
                message.error('服务器异常')
            }
...
export const axiosFormData = (type, url, params, fn) => {
    ...
    axios({
        ...
        if (error.response) {
            if (/^(5|4)\d{2}/.test(error.response.status)) {
                message.error('服务器出现异常, 正在尝试重新上传!')
            }
        } else if (error.request) {
...
export const pcAdPosition = [
    {label: '首页新闻列表嵌入(800px * 140px)', value: '8'},
    {label: '首页中部右侧(320px * 320px)', value: '3'},
    {label: '新闻列表右侧(320px * 320px)', value: '4'}
]
...
export const positionOptions = [
    {label: '首页顶部轮播', value: '1'},
    {label: '首页顶部右侧', value: '2'}
    // {label: '首页产品轮播', value: '8'},
    // {label: '首页活动轮播', value: '9'}
]

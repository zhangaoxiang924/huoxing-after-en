...
import {getAppTopicList, getTopNum, setSearchQuery, setPageData, setFilterData} from '../../actions/appTopic.action'
...
class AppTopicIndex extends Component {
    ...
    componentWillMount () {
        ...
        columns = [{
            ...
            render: (record) => {
                if (record.type === 1) {
                    return <a href={record.typeLink && record.typeLink.trim() !== '' ? record.typeLink : '#/appTopic-list'}>{record.typeLink && record.typeLink.trim() !== '' ? record.typeLink : '暂无链接'}</a>
                } else if (record.type === 2) {
                    return <p>待定</p>
                } else if (record.type === 3) {
                    if (record.typeLink && parseInt(record.typeLink) !== '') {

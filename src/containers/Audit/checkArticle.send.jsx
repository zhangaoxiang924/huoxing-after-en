...
class ArticleAuditSend extends Component {
                    ...
                    confirm({
                        ...
                        onOk () {
                            ...
                            if (_this.state.reason.trim() !== '') {
                                axiosAjax('post', `${_this.state.updateOrNot ? '/news/update' : '/news/add'}`, values, (res) => {
                                    if (res.code === 1) {
                                        axiosAjax('POST', '/news/nopass', {
                                            ...
                                            if (res.code === 1) {
                                                axiosAjax('POST', '/news/status', {id: values.id, status: 2}, (res) => {
                                                    if (res.code === 1) {
    ...
    render () {
        ...
        const { focusImg, allfileList, uploadAllImgModal, previewVisible, previewImage, fileList, pcfileList, mfileList, mcfileList, tags, inputVisible, inputValue, newsContent, updateOrNot, newsVisible } = this.state
        const formItemLayout = {
            labelCol: { span: 1 },
            wrapperCol: { span: 20, offset: 1 }
        }
                ...
                <Form onSubmit={this.handleSubmit}>
                    ...
                    >
                        ...
                        <div>
                            ...
                            <span>建议每篇新闻最多 <font style={{color: 'red'}}>3</font> 个标签, 每个标签最多<font style={{color: 'red'}}> 8 </font>个字</span>
                        </div>
                    </FormItem>
                    ...
                    {this.FormItem('pc', 'PC-封面', 'coverImgUrl', newsInfo, fileList, this.handleChange, '280px * 205px')}
                    {this.FormItem('pc_recommend', 'PC-推荐位', 'pccoverImgUrl', newsInfo, pcfileList, this.handlePcChange, '232px * 220px')}
                    {this.FormItem('wap_small', 'M-缩略图', 'mcoverImgUrl', newsInfo, mfileList, this.handleMobileChange, '145px * 110px')}
                    ...
                    >
                        ...
                        <Button type="primary" id="simditorSub" data-status='1' htmlType="submit" style={{marginRight: '10px', marginBottom: 10}}>审核通过并发表</Button>
                        <Button type="primary" data-status='0' onClick={this.handleSubmit} style={{marginRight: '10px', marginBottom: 10}}>暂存为草稿</Button>

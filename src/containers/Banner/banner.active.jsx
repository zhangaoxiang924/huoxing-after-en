        ...
        columns = [{
            ...
            render: (text, record) => (record && <div className="banner-info clearfix">
                <div>
                    ...
                    {parseInt(record.status) !== 2 ? '' : <div style={{'display': 'inline-block'}}><span className="banner-status has-publish mr10">首页展示中</span></div>}
                </div>
            </div>)
        }, {
            title: '状态',
            ...
            render: (record) => {
                if (record.type === 1) {
                    return <a target="_blank" href={record.typeLink && record.typeLink.trim() !== '' ? 'http://www.marsfinance.net/newsdetail/' + record.typeLink + '.html' : '#/banner-list'}>{`${record.typeLink && record.typeLink.trim() !== '' ? 'http://www.marsfinance.net/newsdetail/' + record.typeLink + '.html' : '暂无链接'}`}</a>
                } else if (record.type === 2) {
